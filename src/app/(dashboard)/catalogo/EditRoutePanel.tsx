...
import { getRouteForEdit, updateRoute, deleteRoute, type NewRoutePrice } from "./actions";
...
export default function EditRoutePanel({
  families,
  routes,
  onClose,
}: {
  ...
  function onSelectRoute(id: string) {
    ...
    startTransition(async () => {
      ...
      setFamily(rt.family ?? "");
      setOrigin(rt.origin ?? "");
      setDestination(rt.destination ?? "");
      setDays(rt.days != null ? String(rt.days) : "");
      setNights(rt.nights != null ? String(rt.nights) : "");
      setKm(rt.km != null ? String(rt.km) : "");
      ...
      for (const p of rt.prices) {
        next[p.modality] = {
          pilgrim: p.price_pilgrim != null ? String(p.price_pilgrim) : "",
          cs: p.price_cs != null ? String(p.price_cs) : "",
        };
      }
      setPrices(next);
      setLoaded(true);
  ...
  function onSave() {
    ...
    startTransition(async () => {
      const r = await updateRoute(routeId, {
        name: name.trim(),
  ...
  function onDelete() {
    setError(null);
    startTransition(async () => {
      const r = await deleteRoute(routeId);
      if (r?.error) setError(r.error);
      else window.location.reload();
    });
  }
  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl p-5 space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="font-display text-lg text-bosque">Editar / eliminar ruta</h3>
      ...
      <label className="text-xs text-muted block max-w-md">
        Ruta
        <select value={routeId} onChange={(e) => onSelectRoute(e.target.value)} className={inputCls}>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <label className="text-xs text-muted">
              Familia (Camino)
              <input list="edit-route-families" value={family} onChange={(e) => setFamily(e.target.value)} className={inputCls} />
              <datalist id="edit-route-families">
            ...
            <label className="text-xs text-muted">
              Dificultad
              <input list="edit-route-difficulties" value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className={inputCls} />
              <datalist id="edit-route-difficulties">
          ...
          <div>
            <p className="text-xs text-muted mb-2">Precios por modalidad — vaciá una fila para quitar ese precio.</p>
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            {confirmDelete ? (
              <div className="flex items-center gap-2 text-sm text-red-800">
                <span>¿Eliminar esta ruta y sus precios e itinerario?</span>
                <button onClick={() => setConfirmDelete(false)} className="text-xs px-3 py-1 rounded-md border border-border hover:bg-taupe/40">Cancelar</button>
                <button onClick={onDelete} disabled={pending} className="text-xs px-3 py-1 rounded-md bg-red-700 text-white hover:bg-red-800 disabled:opacity-50">
                  {pending ? "Eliminando…" : "Sí, eliminar"}
                </button>
              </div>
            ) : (
              <button onClick={() => setConfirmDelete(true)} className="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-md border border-red-300 text-red-700 hover:bg-red-50 transition">
