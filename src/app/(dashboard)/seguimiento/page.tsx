...
export default async function SeguimientoPage() {
  ...
  const contratos = new Map<string, { total: number; sinFirmar: number; empresa: boolean; conjunto: boolean }>();
  for (const c of contracts) {
    const acc = contratos.get(c.quote_id) ?? { total: 0, sinFirmar: 0, empresa: false, conjunto: false };
    acc.total += 1;
    ...
    // Igual el conjunto: un solo contrato para todo el grupo (0054).
    if (c.kind === "conjunto") acc.conjunto = true;
  ...
  const rows: QuoteRow[] = quotes.map((q) => {
    ...
    return {
      ...
      contrato_conjunto: firmas?.conjunto ?? false,
    };
  });
